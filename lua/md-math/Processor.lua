...
function Processor:_on_data(identifier, data_type, data)
    -- require'util'.notify(string.format('received %s %s', identifier, data_type))
...
function Processor:request(data, callback)
    ...
    local code, err = self.pipes[0]:write(identifier .. ':' .. #data .. ':' .. data)
